...
# 📱 Mobile Architecture & Offline-First Engineering Standards (Any-Stack Universal Engine)

> **Consolidated design patterns for high-performance cross-platform and native mobile applications inside `projects/mobile/`, featuring local offline sync engines and OS hardware keystores.**  
> **🌐 Any-Stack Compatibility**: Fully compatible with Flutter (Dart), React Native (TypeScript / Expo), Android Jetpack Compose (Kotlin), iOS SwiftUI (Swift), and Kotlin Multiplatform (KMP).
...
* **Local Master Persistence**: Store critical daily user datasets locally inside embedded relational or object databases (*SQLite, Drift, WatermelonDB, Realm, Hive, CoreData, Room*). The UI rendering view reads EXCLUSIVELY from this local persistence store.
...
Whenever native capabilities, background sync tables, or secure vaults are added to `projects/mobile/`, record their structures in **[mobile-memory.md](../memory/mobile-memory.md)**!
