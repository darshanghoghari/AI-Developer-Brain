...
# ⚙️ Backend Engineering & Cloud Infrastructure Standards (Any-Stack Universal Engine)

> **Consolidated architectural principles for high-performance API microservices, serverless compute, asynchronous event queue processing, and containerized deployment pipelines inside `projects/backend/`.**  
> **🌐 Any-Stack Compatibility**: Fully compatible with Node.js (NestJS, Express), Python (FastAPI, Django), Go (Gin, Fiber), Rust (Actix, Axum), Java (Spring Boot), C# (.NET), and PHP (Laravel).

---

## 1. Strict Layered Architectural Decoupling

Backend applications MUST rigidly enforce separation of concerns across three boundaries regardless of programming language:
...
3. **Repository / Data Layer**: Encapsulate ORM execution (*Drizzle, Prisma, SQLAlchemy, GORM, Hibernate, Entity Framework*) and raw database transaction sessions.
...
## 2. Asynchronous Task Queue Offloading (Redis / RabbitMQ)
...
* **Offloaded Workloads**: Batch email notification dispatching, invoice PDF document rendering, video/media transcoding, and webhooks MUST be pushed to asynchronous queues (*BullMQ, Celery, RabbitMQ, AWS SQS, Kafka*).
...
* **Multi-Stage Docker builds**: Always employ multi-stage compilation in Dockerfiles to isolate source build toolchains from final minimal production runtimes (*e.g., node:20-alpine, python:3.12-slim, or distroless images*), keeping container footprints under 150MB.
...
When deploying or modifying backend APIs in `projects/backend/`, autonomous agents MUST record endpoint routes, schema mappings, and Redis queue names into **[backend-memory.md](../memory/backend-memory.md)**!
