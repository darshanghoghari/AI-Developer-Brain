...
category: "standards"
targetProject: "projects/backend/ & projects/admin/"
version: "3.0.0"
---

# 🔒 Database Modeling & Active Security Shield (Any-Stack Universal Engine)

> **Consolidated high-density engineering standards for relational database optimization, index design, OWASP Top 10 immunity, and cryptographic credential defenses across ANY stack.**

---

## 1. Database Modeling & Query Optimization (PostgreSQL / MySQL / NoSQL)
...
* **Safe Migration Discipline**: NEVER execute manual DDL schema alterations directly on live staging or production clusters. Generate automated, reproducible, bidirectional migration scripts (*e.g., via Drizzle, Prisma, Alembic, or Flyway migration engines*).
...
Never format or concatenate raw string variables directly into database execution expressions in Python, TypeScript, Go, or PHP:
```typescript
...
* Never commit API keys, AWS IAM secrets, database connection strings, or cryptographic salts directly inside git-tracked code files. Reference runtime environment variables (`process.env.DATABASE_URL`, `os.environ.get('DB_URL')`) validated during system startup via schemas (*Zod / Pydantic*).
...
When configuring security firewalls or database migration tables, record structural updates into **[backend-memory.md](../memory/backend-memory.md)** and **[admin-memory.md](../memory/admin-memory.md)**!
